"""
``ContainerAware`` mixin: adds Apptainer container wrapping to host executors.
...
list and a task's option dict, return the command wrapped in ``apptainer
exec ...`` if a container image is resolved, else the command unchanged. It
does not interact with job submission, monitoring, or scheduling — those
...
option cache hash, §4.3 of the orthogonality handover spec).
"""
...
class ContainerAware:
    ...
    def _load_container_config(self, config: Optional[SectionProxy]) -> None:
        ...
        Reads the keys ``default_container``, ``default_bind``, and
        ``default_passthrough_env``. The latter two are comma-separated.
        Missing keys yield the sentinel defaults (no container; empty lists).
        """
        if config is None:
            return
    ...
    def _wrap_command_for_container(
        ...
        """Wrap ``cmd`` in an ``apptainer exec`` invocation if appropriate.

        Returns the wrapped command, or ``cmd`` unchanged if no container is
        resolved. Binds and environment-variable names are sorted lexically
        for reproducibility.
        """
